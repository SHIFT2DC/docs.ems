...
// NOTE: requires `npm install three` in your Docusaurus project
// Wrap with <BrowserOnly> in Docusaurus pages to avoid SSR issues
...
export default function FloatingLines({
  ...
  const targetMouseRef = useRef(null);
  const currentMouseRef = useRef(null);
  const targetInfluenceRef = useRef(0);
  const currentInfluenceRef = useRef(0);
  const targetParallaxRef = useRef(null);
  const currentParallaxRef = useRef(null);
  ...
  const getDist = (waveType) => {
    ...
    return (lineDistance[enabledWaves.indexOf(waveType)] ?? 5) * 0.01;
  };
  ...
  useEffect(() => {
    ...
    import('three').then((mod) => {
      ...
      targetMouseRef.current = new THREE.Vector2(-1000, -1000);
      currentMouseRef.current = new THREE.Vector2(-1000, -1000);
      targetParallaxRef.current = new THREE.Vector2(0, 0);
      currentParallaxRef.current = new THREE.Vector2(0, 0);
      ...
      const handleLeave = () => { targetInfluenceRef.current = 0; };
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: '100%', height: '100%', overflow: 'hidden', mixBlendMode, ...style }}
    />
  );
}